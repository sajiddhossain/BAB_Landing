      ...
      <div className="max-w-3xl mx-auto w-full">
        ...
        <div className="relative flex flex-col gap-16 mb-24 w-full">
          ...
          {modules.map((m, idx) => {
             ...
             return (
               <div key={idx} className={`flex flex-col gap-6 items-start w-full relative ${rotations[idx]} hover:rotate-0 transition-transform duration-300 z-10 ${m.hasWidget ? 'bg-[#FFDE4D]' : bgColors[idx]} border-[4px] border-black p-6 sm:p-10 shadow-[12px_12px_0_0_#0F0F12] text-[#0F0F12]`}>
                 ...
                 {m.hasWidget && (
                   <div className="bg-white border-[4px] border-black p-6 flex flex-col items-center gap-6 shadow-[inset_6px_6px_0_rgba(0,0,0,0.1)] w-full mt-4 rotate-1">
                      ...
                      <div className="flex gap-4">
                        <button onClick={() => setTamagotchiState('active')} className={`font-black text-xl px-6 py-2 border-[3px] border-black rounded-lg transition-all ${tamagotchiState === 'active' ? 'bg-[#DAE69A] translate-y-1 shadow-none' : 'bg-white border-b-[6px] hover:-translate-y-1 hover:shadow-[0_4px_0_0_#000] active:border-b-[3px] active:translate-y-1 active:shadow-none'}`}>A</button>
